...
const chooser = {
  'list-detail': { icon: '☑', name: 'TodoMVC', hint: 'Models. Rows. Your next small victory.' },
  'owned-widget': { icon: '📻', name: 'Radio Ghostbusters', hint: 'Unplug the ghost wiggles.' },
  'mission-control': { icon: '🚀', name: 'Cheese Patrol', hint: 'Catch a flying Promise.' },
};
export const examples = {
  ...
  body: `<section class="examples-page" id="examples">
    ...
    <section class="example-chooser" id="example-chooser" aria-label="Choose a demo"><div class="example-cards">${listRecipes().map(recipe => `<button id="${recipe.id}" data-example-id="${recipe.id}" class="demo-card" type="button" aria-pressed="false"><span class="example-icon" aria-hidden="true">${chooser[recipe.id].icon}</span><span class="example-card-copy"><strong>${chooser[recipe.id].name}</strong><small>${chooser[recipe.id].hint}</small></span></button>`).join('')}</div></section>
...
<section class="home-demos" id="demos" aria-labelledby="home-demos-title">
  ...
  <div class="home-demo-cards">${listRecipes().map(recipe => `<a class="demo-card" href="/demos/#${recipe.id}"><span class="example-icon" aria-hidden="true">${chooser[recipe.id].icon}</span><span class="example-card-copy"><strong>${chooser[recipe.id].name}</strong><small>${chooser[recipe.id].hint}</small></span></a>`).join('')}</div>
</section>`;
