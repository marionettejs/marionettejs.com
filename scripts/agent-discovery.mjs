...
export async function buildAgentDiscovery({ out, manifest, pages, assets, shell, renderMarkdown }) {
  ...
  const identity = {
    ...
    channel: manifest.channel, publication: 'beta (published on npm)',
  ...
  const indexTasks = tasks.map(([title, routes, purpose]) => `## ${title}\n\n${routes.map(route => { const page = byRoute.get(`docs/${route}`); return `- ${link(page.title, mdPath(page))}`; }).join('\n')}\n\n${purpose}`).join('\n\n');
  ...
  const documents = [];
  async function addDocument({ id, title, section, kind, path, url, sourceUrl, sourceSha256 }) {
    const markdown = await readFile(resolve(out, path.slice(1)), 'utf8');
    documents.push({ id, title, section, kind, url: origin + url, markdownUrl: origin + path, sourceUrl, sourceSha256, sha256: sha256(markdown), markdown });
  }
  ...
  const coverage = { schemaVersion: 1, ...identity, snapshotPages: pages.length, publishedPages: documents.filter(doc => doc.kind === 'guide').length, diagnostics: catalog.diagnostics.length, corpusDocuments: documents.length, sourceAssets: assets.length, checkedLocalLinks: checkedLinks, unresolvedLinks, limitations: ['Coverage counts imported snapshot pages, not every API or application scenario.', 'Local link checks verify file destinations; they do not check fragments or external URLs.', 'The corpus contains published guides and diagnostics; source fixtures and the consumer skill remain linked downloads.', 'Website delivery has not been deployed or verified by this build.'] };
